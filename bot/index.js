...
class StreamBot {
  constructor({streams=[], actions=[], config={}}) {
    ...
    this.actionSwitcher = new ActionSwitcher(actions, {blacklisted: config.blacklistedActions})
  }
  ...
  async messageHandler(message, publisher, ctx) {
    ...
    const messages = await this.actionSwitcher.getMessages(trimmedMessage, ctx)
