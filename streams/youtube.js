...
class YoutubeTargetedMessagePublisher extends stream.AbstractTargetedMessagePublisher {
	...
	sendMessage(message) {
		MessageFormatter.createMessageParts(message, YOUTUBE_MAX_MESSAGE_LENGTH)
			.forEach(messagePart => {
...
class YoutubeStream extends stream.AbstractStream {
	...
	async getChatMessages() {
		...
		if (newMessages.length > 0) {
			const publisher = new YoutubeTargetedMessagePublisher(this.youtube, this.youtubeAuth, this.chatId)
	...
	notifyListenerIfNeeded(chatMessage, publisher) {
		...
		if (publishedAt > this.startTime) {
			if (snippet.type === 'textMessageEvent') {
				const messageText = snippet.textMessageDetails.messageText.split(String.fromCharCode(8203)).join('')
				...
				const messageText = snippet.superChatDetails.userComment.split(String.fromCharCode(8203)).join('')
				this.notifyListeners(messageText, publisher, {superChat: true, value: value})
			}
